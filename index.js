...
// Owner numbers
const ownerNumber = ["923183928892"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("rubabmuzamil~")) {
    console.log("❌ Invalid rubabmuzamil~ session format");
    process.exit(1);
  }
  ...
  console.log("♻️ ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ session restored successfully");
}
...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(htmlPath)) {
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          <h1>🤖 ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ</h1>
          <p class="status">✅ BOT IS CONNECTED</p>
          <p>Type .menu in WhatsApp to see commands of ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ</p>
...
async function connectToWA() {
  ...
  console.log(`🤖 ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ Bot`;
      
      // Image URL for connection message
      const imageUrl = 'https://ibb.co/ymzhC8LZ';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923183928892@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
          // Fallback to text if image fails
          sock.sendMessage(ownerNumber[0] + '923183928892@s.whatsapp.net', { text: aliveMsg });
        });
  ...
  // Anti-call feature
  const callMsg = `⚠️ *ANTI-CALL IS ACTIVE* ⚠️\n\nDear User,\n\nYou have attempted to call the ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ bot. To ensure uninterrupted service, please refrain from calling.\n\nThank you for your understanding.\n\n${botConfig.COPYRIGHT || 'ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ'}`;
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://ibb.co/ymzhC8LZ');
      ...
      if (msg.key && msg.key.remoteJid === 'status@broadcast') {
        ...
        if (config.AUTO_STATUS_MSG === 'true') {
          try {
            ...
            console.log("📖 Status seen by ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ");
...
.resetgoodbye`;
      ...
      if (config.ANTI_LINK === 'true' && isGroup && !isAdmins && !isOwner && !msg.key.fromMe && groupSetting.antilink !== false) {
        ...
        if (linkRegex.test(body)) {
          ...
          reply('⚠️ *Links are not allowed in this group! By ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ*');
        }
      }
...
> _Message was deleted but ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ saved it_ 🔰`;
