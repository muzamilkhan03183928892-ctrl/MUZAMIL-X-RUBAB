...
async (conn, mek, m, {
    ...
    try {
        ...
        reply(`${user} has been kicked out of the group! by : ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ`);
    } catch (e) {
        console.log(e);
        await conn.sendMessage(from, { react: { text: '❌', key: mek.key } })
...
cmd({
    pattern: "kickall",
    desc: "Kicks all non-admin members from the group. by: ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ",
    react: "👏",
    category: "group",
    filename: __filename,
},           
async(conn, mek, m,{from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply}) => {
try{
      if (!isAdmins) return reply(`ONLY ADMINS CAN USE THIS COMMOD 🪄♻️`)
      if (!isOwner) return reply(`SORRY ADMINS YOU R NOT BOT OWNER PLEASE GO AND DEPLOY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ🪄♻️`)
...
if (!isAdmins) return reply('You Are Not A Admin')	
  if (args[1] == 'second') {
                ...
                setTimeout(() => {
                    ...
                    const open = `*OPEN TIME* THE GROUP WAS OPENED BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ TO APPROVED ADMIN\n NOW MEMBERS CAN SEND MESSAGES 🔓`
...
if (!isAdmins) return reply('You Are Not A Admin')	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
cmd({
    ...
    desc: "Mention all members in the group line by line with a custom message.By: ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ",
    category: "main",
    filename: __filename
}, 
async (conn, mek, m, { from, isGroup, participants, text, reply }) => {
    try {
        ...
        console.log(`✅ Tagall command used in Group  ${from}`);
    } catch (e) {
...
if (!isAdmins) return reply('You Are Not A Admin')
const G_MUTE = `*GROUP IS CLOSED BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ BOT OWNER*`;
...
if (!isBotAdmins) return reply('You Not A Bot Owner')
if (!isAdmins) return reply('You Are Not A Admin')
const G_UNMUTE = `*GROUP IS OPEN BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ BOT OWNER*`;
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }}) 
reply('*GROUP ADMIN PROMOTE BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ BOT OWNER*')
} catch (e) {
reply('*ERROR*')
console.log(e)
}
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})
reply('*GROUP ADMIN DEMOTE BY ᴍᴜᴢᴀᴍɪʟ x ʀᴜʙᴀʙ OWNER*') 
} catch (e) {
reply('*ERROR*')
console.log(e)
}
